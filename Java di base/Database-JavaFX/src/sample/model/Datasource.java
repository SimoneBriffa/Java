...
public class Datasource {
    ...
    il punto interrogativo è una funzionalità chiave per quanto riguarda la protezione;
    farà in modo che ciò che viene aggiunto alla stringona di comando sql, eventualmente da tastiera,
    non venga concatenata blandamente ma con un meccanismo di protezione, in modo che un eventuale hacker, pur
    digitando il codice hack per far venire fuori tutti i dati, non avrà successo
    */
    ...
    public static final String QUERY_ALBUMS_BY_ARTIST_ID = "SELECT * FROM " + TABLE_ALBUMS +
            " WHERE " + COLUMN_ALBUM_ARTIST + " = = ORDER BY " + COLUMN_ALBUM_NAME + " COLLATE NO CASE";
    ...
    private static Datasource instance = new Datasource(); //creare l'oggetto quì rende la classe thread safe
    private Datasource(){
        //rendiamo la classe singleton, cioè nessuna classe potrà definirne un'istanza
    ...
    public boolean updateArtistName(int id, String newName){
        try{
            ...
            return affectedRecords == 1; //è un boolean !! (:
    ...
    public List<String> queryAlbumsFromArtist(String artistName, int sortOrder) {
        ...
        try (Statement statement = conn.createStatement(); ResultSet results = statement.executeQuery(sb.toString())) {
            ...
            while (results.next()) {
                albums.add(results.getString(1)); //1 è l'indice della colonna(in questo caso ce ne sarà solo una
    ...
    //la seguente funzione serve per fare ciò che in sql facciamo con .schema
    ...
    public int getCount(String table){
        ...
        try(Statement statement = conn.createStatement(); ResultSet results = statement.executeQuery(sql)){
            ...
            //è buona norma dare un nome al conteggio delle righe
    ...
    //Creare una VIEW con Java non è niente di diverso dal crearla con la command line
    ...
    public int insertArtists(String name) throws SQLException {
        ...
        if(results.next()) { //in questo caso significa che l'artista è già presente nel database
            ...
            if(affectedRows != 1){ //se ritorna 0 vuol dire che non ha aggiunto righe, cioè ha fallito
    ...
    public int insertAlbum(String name, int artistId) throws SQLException {
        ...
        if(results.next()) { //in questo caso significa che l'album è già presente nel database
            return results.getInt(1); //ci ritorna _id di artist
        }else{
            //inseriamo l'album
    ...
    public void insertSong(String title, String artist, String album, int track) {
        ...
        try {
            conn.setAutoCommit(false); //se mettiamo autoCommit su true, il rollback non potrà essere eseguito
